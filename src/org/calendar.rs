...
use crate::client::{CalendarEventInsert, CalendarEventModify};
        ...
        [
                ...
                .flat_map(|(id, events)| {
                    ...
                    match pending.1.get(id) {
                        Some(CalendarEventModify::Patch { event: new_event }) => {
                            str.push_str("<<<<<<< remote (read only)");
                            str.push('\n');
                            str.push_str(&render_event(&event.0, "* COMMENT".to_owned(), true));
                            str.push_str("=======");
                            str.push('\n');
                            str.push_str(&render_event(new_event, "* ".to_owned(), false));
                            str.push_str(">>>>>>> local");
                            str.push('\n');
                        }
            ...
            pending
                ...
                .map(|CalendarEventInsert::Insert { event }| {
                    ...
                    str.push_str("=======");
                    str.push('\n');
                    str.push_str(&render_event(event, "* ".to_owned(), false));
                    str.push_str(">>>>>>> local");
                    str.push('\n');
                    str
                })
