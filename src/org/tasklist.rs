...
use crate::client::{TaskInsert, TaskModify};
        ...
        [
            handle.map_into::<_, Vec<_>, _>(|id, tasks| {
                ...
                match pending.1.get(id) {
                    Some(TaskModify::Patch { task: new_task }) => {
                        ...
                        str.push('\n');
                        str.push_str(&render_task(&task.0, "* COMMENT".to_owned(), true));
                        str.push_str("=======");
                        str.push('\n');
                        str.push_str(&render_task(new_task, "* ".to_owned(), false));
                        str.push_str(">>>>>>> local");
                        str.push('\n');
                    }
            ...
            pending
                ...
                .map(|TaskInsert::Insert { task }| {
                    ...
                    str.push_str("=======");
                    str.push('\n');
                    str.push_str(&render_task(task, "* ".to_owned(), false));
                    str.push_str(">>>>>>> local");
                    str.push('\n');
                    str
                })
