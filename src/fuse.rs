...
use crate::tasklist::OrgTaskList;
...
const fn tasks_file_attr(uid: u32, gid: u32, ino: Inode, size: u64) -> FileAttr {
    FileAttr {
        ino,
        size,
        uid,
...
const OFFSET: Inode = TASKS_DIR_INO + 1;

impl OrgFS {
    pub(crate) fn new(tasklists: Vec<OrgTaskList>) -> Self {
        Self {
            ...
            tasklists: tasklists
                .into_iter()
                .enumerate()
                .map(|(i, tl)| (OFFSET + i as u64, tl))
                .collect(),
        }
    }

    fn is_tasks_file(&self, ino: Inode) -> bool {
        OFFSET <= ino && ino < OFFSET + self.tasklists.len() as Inode
    }
}
            ...
            p if p == TASKS_DIR_INO => match name.to_str() {
                Some(file) => {
                    if let Some(i) = self.tasklists.iter().find(|x| {
                        ...
                        reply.entry(
                            &TTL,
                            &tasks_file_attr(self.uid, self.gid, i.0, i.1.to_org().len() as u64),
                            0,
                        );
                    } else {
                        reply.error(ENOENT);
        ...
        match ino {
            ...
            i if self.is_tasks_file(i) => {
                if let Some((_, tl)) = self.tasklists.iter().find(|(ino, _)| ino == &i) {
                    reply.attr(
                        &TTL,
                        &tasks_file_attr(self.uid, self.gid, i, tl.to_org().len() as u64),
                    )
                } else {
                    reply.error(ENOENT);
                }
